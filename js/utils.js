const btf = {
  ...
  snackbarShow: (text, showActionFunction = false, duration = 2000, actionText = false) => {
    ...
    root.style.setProperty("--icemyst-snackbar-time", duration + "ms");
  ...
  initThemeColor: function () {
    let themeColor = getComputedStyle(document.documentElement)
      .getPropertyValue("--icemyst-bar-background")
      .trim()
      .replace('"', "")
      .replace('"', "");
    ...
    if (currentTop > 26) {
      if (btf.is_Post()) {
        themeColor = getComputedStyle(document.documentElement)
          .getPropertyValue("--icemyst-meta-theme-post-color")
          .trim()
          .replace('"', "")
          .replace('"', "");
      }
  ...
  downloadImage: function (imgsrc, name) {
    ...
    if (rm.downloadimging == false) {
      ...
      btf.snackbarShow("正在下载中，请稍后", false, 5000);
      setTimeout(function () {
        let image = new Image();
        // 解决跨域 Canvas 污染问题
  ...
  getCustomPlayList: function () {
    ...
    if (urlParams.get("id") && urlParams.get("server")) {
      ...
      anMusicPageMeting.innerHTML = `<meting-js id="${id}" server=${server} type="playlist" type="playlist" mutex="true" preload="auto" theme="var(--icemyst-main)" order="list" list-max-height="calc(100vh - 169px)!important"></meting-js>`;
    } else {
      anMusicPageMeting.innerHTML = `<meting-js id="${userId}" server="${userServer}" type="playlist" mutex="true" preload="auto" theme="var(--icemyst-main)" order="list" list-max-height="calc(100vh - 169px)!important"></meting-js>`;
    }
